...
VentasController.createVenta = async (req, res) => {
    ...
    try {
        ...
        for (const item of ventaData.products) {
            const product = await Product_1.Product.findByPk(item.id);
            if (!product) {
        ...
        // Crear la venta
        const nuevaVenta = await Venta_1.Venta.create({
...
VentasController.getVentas = async (req, res) => {
    try {
        const ventas = await Venta_1.Venta.findAll();
...
VentasController.getVentaById = async (req, res) => {
    ...
    try {
        const venta = await Venta_1.Venta.findByPk(id);
        if (!venta) {
...
VentasController.deleteVenta = async (req, res) => {
    ...
    try {
        ...
        for (const item of productos) {
            const producto = await Product_1.Product.findByPk(item.id);
            if (producto) {
...
VentasController.updateComment = async (req, res) => {
    const id = req.params.id;
